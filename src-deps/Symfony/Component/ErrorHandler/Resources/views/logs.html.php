...
<?php $channelIsDefined = isset($logs[0]['channel']); ?>
    ...
    foreach ($logs as $log) {
        if ($log['priority'] >= 400) {
            ...
            $severity = 0;
            if (($exception = $log['context']['exception'] ?? null) instanceof \ErrorException || $exception instanceof \Enpii_Base\Deps\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
